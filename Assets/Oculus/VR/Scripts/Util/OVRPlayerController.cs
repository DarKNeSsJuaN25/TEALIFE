...
public class OVRPlayerController : MonoBehaviour
{
    ...
    {
        isenableMovement = enable;
        EnableLinearMovement = enable;
        EnableRotation = enable;
    }
    public bool isEnabledMovement()
    {
        return isenableMovement;
    }
    ...
    public virtual void UpdateMovement()
    {
        ...
        if (EnableRotation)
        {
            // Define la influencia de la rotación basada en el tiempo y la configuración
            float rotateInfluence = SimulationRate * Time.deltaTime * RotationAmount * RotationScaleMultiplier;

            // Obtiene la entrada del eje del joystick derecho
            Vector2 secondaryAxis = OVRInput.Get(OVRInput.Axis2D.SecondaryThumbstick);

            // Aplica la rotación suave basada en la entrada del joystick derecho
